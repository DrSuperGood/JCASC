...
public abstract class Hex {
	...
	public static byte[] decodeHex(final CharSequence hex) {
		final var nibbleCount = hex.length();
		var valueNibbleShift = (nibbleCount - 1) % (Byte.SIZE / RADIX_SIZE) * RADIX_SIZE;
		final byte[] values = new byte[nibbleCount + 1 >> 1];
		var valueIndex = 0;
		var value = 0;

		for (var nibbleIndex = 0 ; nibbleIndex < nibbleCount ; nibbleIndex+= 1) {
			final var nibble = decodeNibble(hex.charAt(nibbleIndex));
			if (nibble == NO_VALUE) {
