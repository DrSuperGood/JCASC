...
public class TVFSDecoder {
	...
	private List<PathNode> decodeContainer(final ByteBuffer pathBuffer)
			throws MalformedCASCStructureException {
		final var nodes = new ArrayList<PathNode>();

		while (pathBuffer.hasRemaining()) {
			final var node = decodeNode(pathBuffer);
			nodes.add(node);
		}
	...
	private PathNode decodeNode(final ByteBuffer pathBuffer) throws MalformedCASCStructureException {
		...
		try {
			...
			if ((value & VALUE_CONTAINER_FLAG) != 0) {
				...
				final var containerBuffer = pathBuffer.slice();
				...
				final var nodes = decodeContainer(containerBuffer);

				node = new PrefixNode(pathFragments, nodes);
			} else {
				// file value
				final var fileReferences = getFileReferences(value);
	...
	private StorageReference[] getFileReferences(final int fileOffset) throws MalformedCASCStructureException {
		...
		try {
			final var referenceCount = Byte.toUnsignedInt(logicalBuffer.get());
	...
	public TVFSFile loadFile(final ByteBuffer fileBuffer) throws IOException {
		final var localBuffer = fileBuffer.slice();
		...
		try {
			...
			final var headerSize = Byte.toUnsignedInt(localBuffer.get());
		...
		final var rootNodes = decodeContainer();
		final var tvfsFile = new TVFSFile(version, flags, encodingKeySize, patchKeySize, maximumPathDepth, rootNodes);
