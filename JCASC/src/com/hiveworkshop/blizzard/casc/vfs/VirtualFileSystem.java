...
public final class VirtualFileSystem {
	...
	public final class PathResult {
		...
		public boolean existsInStorage() {
			...
			if (isFile()) {
				final var fileNode = (FileNode) node;
				final var fileReferenceCount = fileNode.getFileReferenceCount();
		...
		public boolean isTVFS() {
			...
			final var fileNode = (FileNode) node;
			final var fileReference = fileNode.getFileReference(0);
		...
		public ByteBuffer readFile(ByteBuffer destBuffer) throws IOException {
			...
			final var fileSize = getFileSize();
			...
			final var fileBuffer = destBuffer.slice();
	...
	private static int compareNodePathFragments(final byte[][] pathFragments, final int fragmentIndex,
		...
		final var nodeFragmentCount = node.getPathFragmentCount();
		if (nodeFragmentCount == 0) {
			// nodes without fragments have no path fragment presence so always match
			return 0;
		}

		final var nodeFragment = node.getFragment(0);
		final var fragment = pathFragments[fragmentIndex];
		...
		return Arrays.compareUnsigned(fragment, fragmentOffset,
	...
	public static byte[][] convertFilePath(final String filePath) throws CharacterCodingException {
		final var fragmentStrings = filePath.toLowerCase(Locale.ROOT).split("\\" + PATH_SEPERATOR);
		final var pathFragments = new byte[fragmentStrings.length][];

		final var encoder = PATH_ENCODING.newEncoder();
		encoder.onMalformedInput(CodingErrorAction.REPORT);
		encoder.onUnmappableCharacter(CodingErrorAction.REPORT);
		for (var index = 0; index < fragmentStrings.length; index += 1) {
			final var fragmentBuffer = encoder.encode(CharBuffer.wrap(fragmentStrings[index]));
			if (fragmentBuffer.hasArray() && fragmentBuffer.limit() == fragmentBuffer.capacity()
				...
				// copy into array
				final var pathFragment = new byte[fragmentBuffer.remaining()];
	...
	public static String convertPathFragments(final byte[][] pathFragments) throws CharacterCodingException {
		final var fragmentStrings = new String[pathFragments.length];

		final var decoder = PATH_ENCODING.newDecoder();
	...
	private static boolean equalNodePathFragments(final byte[][] pathFragments, int fragmentIndex, int fragmentOffset,
		...
		while (result && nodeFragmentIndex < nodeFragmentCount) {
			final var fragment = pathFragments[fragmentIndex + nodeFragmentIndex];
			final var nodeFragment = node.getFragment(nodeFragmentIndex);
			result = result && Arrays.equals(fragment, fragmentOffset,
	...
	public VirtualFileSystem(final Storage storage, final Map<String, String> buildConfiguration) throws IOException {
		...
		while (buildConfiguration
			...
			final var storageReference = new StorageReference(configurationKey, buildConfiguration);
			tvfsStorageReferences.put(storageReference.getEncodingKey(), storageReference);
		}

		final var rootReference = new StorageReference(CONFIGURATION_KEY_PREFIX + ROOT_KEY, buildConfiguration);
		final var rootBuffer = fetchStoredBuffer(rootReference);
	...
	private ByteBuffer fetchStoredBuffer(final StorageReference storageReference) throws IOException {
		final var size = storageReference.getSize();
		if (size > Integer.MAX_VALUE) {
			throw new MalformedCASCStructureException("stored data too large to process");
		}

		final var bankStream = storage.getBanks(storageReference.getEncodingKey());
		final var storedBuffer = ByteBuffer.allocate((int) size);
		try {
			while (bankStream.hasNextBank()) {
	...
	public List<PathResult> getAllFiles() throws IOException {
		final var pathStringList = new ArrayList<PathResult>();

		final var rootCount = tvfsRoot.getRootNodeCount();
		for (var rootIndex = 0; rootIndex < rootCount; rootIndex += 1) {
			final var root = tvfsRoot.getRootNode(rootIndex);
	...
	private void recursiveFilePathRetrieve(final byte[][] parentPathFragments, final ArrayList<PathResult> resultList,
		...
		final var fragmentCount = currentNode.getPathFragmentCount();
		if (fragmentCount > 0) {
			var fragmentIndex = 0;
			final var fragment = currentNode.getFragment(fragmentIndex++);

			// expand path fragment array
			var basePathFragmentsIndex = currentPathFragments.length;
			...
			// merge fragment
			final var sourceFragment = currentPathFragments[basePathFragmentsIndex];
			var joinedFragment = fragment;
			if (sourceFragment != null) {
				joinedFragment = sourceFragment;
				if (fragment.length != 0) {
					final var joinOffset = sourceFragment.length;
		...
		if (currentNode instanceof PrefixNode) {
			final var prefixNode = (PrefixNode) currentNode;
			...
			final var fileNode = (FileNode) currentNode;

			final var fileReferenceCount = fileNode.getFileReferenceCount();
			if (fileReferenceCount == 1) {
				// check if nested VFS
				final var encodingKey = fileNode.getFileReference(0).getEncodingKey();
				final var tvfsFile = resolveTVFS(encodingKey);

				if (tvfsFile != null) {
					// file is also a folder
					final var folderPathFragments = Arrays.copyOf(currentPathFragments,
							currentPathFragments.length + 1);
					folderPathFragments[currentPathFragments.length] = new byte[0];

					final var rootCount = tvfsFile.getRootNodeCount();
					for (var rootIndex = 0; rootIndex < rootCount; rootIndex += 1) {
						final var root = tvfsFile.getRootNode(rootIndex);
	...
	private FileNode recursiveResolvePathFragments(final byte[][] pathFragments, int fragmentIndex, int fragmentOffset,
		...
		if (node instanceof PrefixNode) {
			...
			final var prefixNode = (PrefixNode) node;
			final var childCount = prefixNode.getNodeCount();
	...
	public PathResult resolvePath(final byte[][] pathFragments) throws IOException {
		...
		final var result = recursiveResolvePathFragments(pathFragments, 0, 0, tvfsRoot.getRootNode(0));
		if (result == null) {
	...
	private TVFSFile resolveTVFS(final Key encodingKey) throws IOException {
		...
		final var storageReference = tvfsStorageReferences.get(encodingKey);
		if (storageReference != null) {
			...
			synchronized (this) {
				...
				if (tvfsFile == null) {
					...
					final var rootBuffer = fetchStoredBuffer(storageReference);
