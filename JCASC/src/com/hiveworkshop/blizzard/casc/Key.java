...
public final class Key implements Comparable<Key> {
	...
	public int compareTo(final Key o) {
		final var commonLength = Math.min(key.length, o.key.length);
		return Arrays.compareUnsigned(key, 0, commonLength, o.key, 0, commonLength);
	}
	...
	public boolean equals(final Object obj) {
		...
		final var otherKey = (Key)obj;
		final var commonLength = Math.min(key.length, otherKey.key.length);
		if (!Arrays.equals(key, 0, commonLength, otherKey.key, 0, commonLength)) {
			return false;
		}
	...
	public String toString() {
		final var builder = new StringBuilder((key.length + 1));
