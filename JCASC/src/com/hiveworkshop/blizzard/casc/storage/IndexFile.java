...
public class IndexFile {
	...
	private void decode(final ByteBuffer fileBuffer) throws IOException {
		final var sourceBuffer = fileBuffer.slice();
		
		// decode header
		
		final var hashBlockProcessor = new LittleHashBlockProcessor();
		...
		final var entriesAlignmentMask = ENTRY_BLOCK_ALIGNMENT - 1;
		...
		final var entryLength = fileSizeLength + dataOffsetLength + encodingKeyLength;
		final var entryCount = entryBuffer.remaining() / entryLength;

		entries.ensureCapacity(entryCount);

		final var decodeDataOffsetBuffer = ByteBuffer.allocate(Long.BYTES);
		final var decodeDataOffsetOffset = Long.BYTES - dataOffsetLength;
		final var decodeFileSizeBuffer = ByteBuffer.allocate(Long.BYTES);
		decodeFileSizeBuffer.order(ByteOrder.LITTLE_ENDIAN);
		for (var i = 0; i < entryCount; i += 1) {
			final var key = new byte[encodingKeyLength];
			entryBuffer.get(key);

			entryBuffer.get(decodeDataOffsetBuffer.array(), decodeDataOffsetOffset, dataOffsetLength);
			final var dataOffset = decodeDataOffsetBuffer.getLong(0);

			entryBuffer.get(decodeFileSizeBuffer.array(), 0, fileSizeLength);
			final var fileSize = decodeFileSizeBuffer.getLong(0);
	...
	public IndexEntry getEntry(final Key encodingKey) {
		final var index = Collections.binarySearch(entries, encodingKey, (left, right) -> {
			if (left instanceof IndexEntry && right instanceof Key) {
				final var entry = (IndexEntry)left;
				final var ekey = (Key)right;
